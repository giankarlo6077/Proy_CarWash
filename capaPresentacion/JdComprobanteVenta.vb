...
Public Class ComprobanteVenta
    ...
    Private Sub ComprobanteVenta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For i As Integer = 0 To cbxTipoServicio.Items.Count - 1
            ...
            End If
        Next
        ' El tipo de servicio lo fija el formulario llamador y no se puede cambiar
    ...
    ' ─────────────────────────────────────────────
    '  GUARDAR / MODIFICAR COMPROBANTE (botón dual)
    ' ─────────────────────────────────────────────
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click

        If btnGuardar.Text = "Guardar Comprobante" Then
            ...
            Try
                ...
                ' Cambiar texto del botón y bloquear controles
        ...
        ElseIf btnGuardar.Text = "Modificar Comprobante" Then
            ' ── Confirmar antes de modificar ─────────────
            Dim confirmar As DialogResult = MessageBox.Show(
                "¿Desea modificar el comprobante?",
                "Confirmar",
                MessageBoxButtons.YesNo,
                MessageBoxIcon.Question)
            If confirmar = DialogResult.No Then Return

            ' Re-habilitar controles y volver al modo edición
            ...
            If Not datosPendientes Then cbxTipoServicio.Enabled = True
            Dim esEfectivo As Boolean = (cbxMedioPago.Text.Trim().ToLower() = "efectivo")
            txtMonto.Enabled = esEfectivo
            txtMonto.BackColor = If(esEfectivo,
                                    System.Drawing.SystemColors.Window,
    ...
    Private Sub agregarProductoEnDetalle(nombre As String, precio As Decimal, cantidad As Integer)
        For Each fila As DataGridViewRow In DataGridView1.Rows
            If Not fila.IsNewRow AndAlso
               Convert.ToString(fila.Cells("Producto").Value) = nombre Then
                Dim cantActual As Integer = Convert.ToInt32(fila.Cells("Cantidad").Value)
                Dim precioUnit As Decimal = Convert.ToDecimal(fila.Cells("Precio").Value)
                Dim nuevaCant As Integer = cantActual + cantidad
                fila.Cells("Cantidad").Value = nuevaCant
                fila.Cells("Subtotal").Value = (nuevaCant * precioUnit).ToString("0.00")
                calcularTotales()
                Return
            End If
        Next
        DataGridView1.Rows.Add(nombre, cantidad, precio, (cantidad * precio).ToString("0.00"))
        calcularTotales()
    End Sub
    ...
    ' ─────────────────────────────────────────────
    '  AGREGAR PRODUCTO EN MODO MODIFICACIÓN
    ' ─────────────────────────────────────────────
    Private Sub btnAgregarProducto_Click(sender As Object, e As EventArgs) Handles btnAgregarProducto.Click
        Dim frmSel As New JdSeleccionarProductoVenta()
        AddHandler frmSel.ProductoAgregado, AddressOf OnProductoAgregado
        frmSel.ShowDialog(Me)
        RemoveHandler frmSel.ProductoAgregado, AddressOf OnProductoAgregado
    End Sub

    Private Sub OnProductoAgregado(nombre As String, precio As Decimal, cantidad As Integer)
        Dim cant As Decimal = cantidad
        Dim pre As Decimal = precio
        DataGridView1.Rows.Add(nombre, cant, pre, (cant * pre).ToString("0.00"))
        calcularTotales()
    End Sub
